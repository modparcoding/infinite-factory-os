import { z } from "zod";
import { RelationshipHeuristicSnapshotSchema, RelationshipStateSchema, } from "@ifos/contracts";
...
function deriveBaseTrustScore(input) {
...
function pickTrustLevel(input) {
    ...
    if (input.repetition_risk >= 0.5 && input.commercial_density >= 0.55) {
        ...
        return { trustLevel: "saturated", steps, trust_score: adjustedTrust };
    }
    ...
    if (adjustedTrust >= 0.85) {
        ...
        return { trustLevel: "trusting", steps, trust_score: adjustedTrust };
    }
    if (adjustedTrust >= 0.6) {
        steps.push("engaged_threshold_met");
        return { trustLevel: "engaged", steps, trust_score: adjustedTrust };
    }
    if (adjustedTrust >= 0.35) {
        steps.push("warming_threshold_met");
        return { trustLevel: "warming", steps, trust_score: adjustedTrust };
    }
    steps.push("cold_fallback");
    return { trustLevel: "cold", steps, trust_score: adjustedTrust };
}
function validateTransition(previousTrustLevel, nextTrustLevel, trustScore) {
    if (!previousTrustLevel || previousTrustLevel === nextTrustLevel) {
        return;
    }
    const trustOrder = ["cold", "warming", "engaged", "trusting"];
    const indexOf = (value) => trustOrder.indexOf(value);
    if (trustOrder.includes(previousTrustLevel) && trustOrder.includes(nextTrustLevel)) {
        const previousIndex = indexOf(previousTrustLevel);
...
export function deriveRelationshipStateFromInput(input) {
...
export function deriveRelationshipStateFromMemory(memory) {
    ...
    const trustScoreEstimate = clampValue(Math.min(1, (Math.min(1, memory.engagement_rate / 15) * 0.55)
