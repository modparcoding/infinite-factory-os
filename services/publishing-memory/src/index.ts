...
function normalizeBuckets(items: string[]): BucketCounts {
...
function bucketFromEvents<T>(events: T[], key: (event: T) => string): BucketCounts {
  return normalizeBuckets(events.map(key));
}

function assertSingleCampaignAndJourney(events: PublishingEvent[]) {
...
function sortPublishingEvents(events: PublishingEvent[]): PublishingEvent[] {
...
function computeCommercialDensity(events: PublishingEvent[]): number {
  const ratio = events.filter((item) => item.commercial_signal).length / events.length;
  return Number(ratio.toFixed(6));
}

function trendFromSeries(series: number[]): { delta: number; direction: "up" | "down" | "flat"; movement_strength: number } {
  if (series.length < 2) {
...
function computeRepetitionRisk(events: PublishingEvent[]): number {
  if (events.length <= 1) {
    return 0;
  }
...
function deriveEngagementRate(events: PublishingEvent[]): number {
  if (events.length === 0) {
    return 0;
  }
...
export function buildPublishingMemory(snapshotInput: PublishingMemoryInputFixture): PublishingMemorySnapshot {
  ...
  const payload: PublishingMemorySnapshot = {
...
export function evaluatePublishingMemory(input: unknown): PublishingMemorySnapshot {
  const parsed = PublishingMemoryInputFixtureSchema.parse(input);
  return buildPublishingMemory(parsed);
}

export function evaluatePublishingMemoryFailureCase(sourceEvents: unknown[], expectedFailure: string): boolean {
...
export function normalizePublishingEventRows(events: unknown[]): PublishingEvent[] {
